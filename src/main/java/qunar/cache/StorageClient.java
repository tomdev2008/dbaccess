...
import qunar.cache.exception.StorageException;
import qunar.util.Continuum;
import qunar.util.SerializeUtil;
...
public class StorageClient extends NodeDataListener implements CacheAccess {
    ...
    /**
     * public Object getObject(String key) throws CacheException { Object
     * obj = null; byte[] byteVal = get(key); if (byteVal != null &&
     * byteVal.length != 0) { try { obj =
     * SerializeUtil.deserialize(byteVal); } catch (Exception e) { throw
     * new StorageException(description("StorageClient.getObject() key:" +
     * key)); } } return obj; }
     */
    ...
    /**
     * public boolean setObject(String key, Object val, int expireTime)
     * throws CacheException { if (val == null) { return false; } if (!(val
     * instanceof Serializable)) { throw new StorageException(
     * description("value object doesn't impl java.io.Serializable")); }
     * try { byte[] byteVal = SerializeUtil.serialize(val); return set(key,
     * byteVal, expireTime); } catch (Exception e) {
     * logger.error(description("StorageClient.setObject() key:" + key +
     * "\tvalue:" + val + "\texpireTime:" + expireTime)); throw new
     * StorageException(description(e.getMessage()), e); } }
     */
    ...
    /**
     * @Override public Map<String, byte[]> multiGet(List<String> keys)
     *           throws CacheException { //TODO return null; }
     */
