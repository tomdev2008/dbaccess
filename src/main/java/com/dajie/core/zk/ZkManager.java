...
public class ZkManager {
	...
	public void exists(String znodePath) {
		synchronized (watcher) {
			try {
				...
				logger.error("ZkManager.exists() znodePath:" + znodePath + "\t"
						+ e.toString());
			} catch (InterruptedException e) {
				logger.error("ZkManager.exists() znodePath:" + znodePath + "\t"
						+ e.toString());
	...
	public List<String> getChildren(String znodePath) {
		...
		synchronized (watcher) {
			try {
				...
				logger.error("ZkManager.getChildren() znodePath:" + znodePath
						+ "\t" + e.toString());
			} catch (InterruptedException e) {
				logger.error("ZkManager.getChildren() znodePath:" + znodePath
						+ "\t" + e.toString());
	...
	private class ZkWatcher implements Watcher {
		...
		public void process(WatchedEvent event) {
			...
			logger.info("ZkManager.ZkWatcher.process get notify : path = "
					+ event.getPath() + " type = " + event.getType()
					+ " state = " + event.getState());
			if (!isAlive) {
				logger.error("ZkManager.ZkWatcher.process get notify : path = "
						+ event.getPath() + " type = " + event.getType()
						+ " state = " + event.getState()
					...
					while (true) {
						try {
							try {
								...
								logger.error("ZkManager.ZkWatcher.process close "
										+ e);
							}
							isAlive = false;
							...
							notifyTask
									.addMessage(ZkClient.UPDATE_ALL_NAMESPACE);
							return;
						} catch (Throwable e) {
							logger.error("ZkManager.ZkWatcher.process reCreate "
									+ e);
							try {
								TimeUnit.SECONDS.sleep(1);
							} catch (InterruptedException e1) {
								e1.printStackTrace();
								logger.error("ZkManager.ZkWatcher.process Thread.sleep "
										+ e);
								continue;
							}
