...
import sirius.zk.ZNodeListener;
import sirius.zk.ZkClient;
import sirius.zk.ZookeeperException;
...
public class DbConfig extends ZNodeListener implements ConnectionAccess {
    ...
    public DbConfig(String bizName) {
        ...
        gate = new Object();
        init();
        logger.debug("bizName:" + bizName + "\treadEntryList.size():" + readEntryList.size()
                + "\twriteEntryList.size():" + writeEntryList.size());
    }

    public void init() {
        try {
            List<String> contentList = ZkClient.getInstance().getNodes(this.getZNode());
            update(contentList);
    ...
    public boolean update(List<String> childrenNameList) {
        ...
        for (String nodeString : childrenNameList) {
            try {
                ...
                Entry entry = new Entry(host, port, user, password, flag, patternStr, dbName,
                        coreSize, maxSize);
                if (entry.isWritable()) {
    ...
    private static class Entry {
        ...
        public Entry(String host, int port, String user, String password, String rwFlag,
